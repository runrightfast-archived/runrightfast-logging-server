 ...
 *          RRF_HTTP_PORT, but if not specified, then uses the specified value
 *          or 8000 if not port is specified
 */
...
module.exports.logLevel = function logLevel(defaultValue) {
	if (process.env.RRF_LOG_LEVEL) {
		return process.env.RRF_LOG_LEVEL;
	}
